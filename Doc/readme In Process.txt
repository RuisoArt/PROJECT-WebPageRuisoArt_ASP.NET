...
> Shopping Cart
	...
	Para crear nuestro carrito de compras debemos hacer una serie de modificaciones y creacion de nuevos modelos controladores vistas y athenticaciones que seran necesarias para llevar nuestra compra a successful.
	como siempre en este readme invitamos a observar los codigos que en el proyecto se encuentran pues asqui solo se relata el orden de los elementos que en el proyecto se crean.
	...
	Models
		ShoppingCartItem.cs (crear)

	Data
		ApplicationDbContext.cs (modificar, agregar nuevos modelos)

	Data
		Cart
			ShoppingCart.cs (crear, es una especie de controlador)

	Data
		ViewModels
			ShoppingCartVM.cs (crear, es una vista especial)

	Controllers
		ProductController.cs (modificar){

				// Lo siguiente es lo que se debe agregar
	...
	Controllers
		OrderController.cs (crear)

	Views
		Order
			ShoppingCart.cshtml (crear)
	...
	Views
		Product
			Index.cshtml (modificar)

	Views
		Product
			Details.cshtml (modificar)

	Data
		ViewComponent
			ShoppingCartSummary.cs (crear)

	Views
		Shared
			Components
				ShoppingCartSummary
					Default.cshtml (crear)
	...
	Views
		Order
			OrderCompleted.cshtml (crear)

	Hay que tener en cuenta que en varias de las vistas se modificaron aspectos meramente visuales y para lograrlo se necesito crear archivos.cshtml.cs para conseguirlo.
